...
import { Target, Lightbulb, Users, Briefcase, Shield } from "lucide-react";
  ...
  const operationalPlan = [
    {
      ...
      duration: "4 mois",
      actions: ["Travaux VRD", "Installation équipements", "Mise en service", "Formation"]
    },
    {
            ...
            {strategicObjectives.map((objective, index) => (
              <div key={index} className="p-4 border rounded-lg hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <Badge variant="outline">{objective.timeline}</Badge>
                </div>
        ...
        <CardContent>
          <div className="grid md:grid-cols-2 gap-4">
            {marketingStrategy.map((strategy, index) => (
              <div key={index} className="p-4 bg-green-50 rounded-lg">
                <div className="flex justify-between items-start mb-2">
                  <h4 className="font-semibold text-green-800">{strategy.action}</h4>
                  <Badge className="bg-green-200 text-green-800">{strategy.budget}</Badge>
                </div>
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            <div className="flex justify-between items-center">
              <span className="font-semibold">Budget Marketing Total</span>
              <span className="text-xl font-bold text-green-600">31 000€</span>
            </div>
          </div>
        </CardContent>
      </Card>
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      {phase.actions.map((action, actionIndex) => (
                        <div key={actionIndex} className="text-sm bg-purple-50 text-purple-700 px-2 py-1 rounded">
                          {action}
                        </div>
                      ))}
                    </div>
              ...
              <div key={index} className="p-4 border rounded-lg">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <Badge 
                    variant="outline" 
                    className={`
                      ${risk.color === 'red' ? 'border-red-300 text-red-700' : ''}
                      ${risk.color === 'yellow' ? 'border-yellow-300 text-yellow-700' : ''}
                      ${risk.color === 'green' ? 'border-green-300 text-green-700' : ''}
                    `}
                  >
                    Impact {risk.impact}
                  </Badge>
      ...
      <Card className="border-l-4 border-l-cyan-500">
        ...
        <CardContent>
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              <h4 className="font-semibold mb-3">Phase de Lancement</h4>
              <ul className="space-y-2 text-sm">
                <li className="flex justify-between">
                  <span>Gérant/Responsable</span>
                  <span className="font-medium">1 poste (vous)</span>
                </li>
                <li className="flex justify-between">
                  <span>Agent d'accueil/maintenance</span>
                  <span className="font-medium">1 poste temps partiel</span>
                </li>
                <li className="flex justify-between">
                  <span>Prestataire maintenance</span>
                  <span className="font-medium">Contrat Istobal</span>
                </li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold mb-3">Phase de Développement</h4>
              <ul className="space-y-2 text-sm">
                <li className="flex justify-between">
                  <span>Équipe élargie</span>
                  <span className="font-medium">2-3 employés</span>
                </li>
                <li className="flex justify-between">
                  <span>Horaires étendus</span>
                  <span className="font-medium">7j/7 en saison</span>
                </li>
                <li className="flex justify-between">
                  <span>Formation continue</span>
                  <span className="font-medium">Certifications éco</span>
                </li>
              </ul>
            </div>
          </div>
